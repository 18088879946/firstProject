window.addEventListener('load', function () {
    ...
    var swiper_slide = box.querySelector('.swiper-slide');
    // var pul = document.querySelector('.pot-ul');
    // var potli = pul.querySelectorAll('.pot-li');
    // var center = box.querySelector('.center');
    // var swiper = center.querySelector('.swiper');
    ...
    timer = setInterval(function () {
        ...
        swiper_slide.style.transition = 'all .5s';
        swiper_slide.style.transform = 'translateX(' + translatex + 'px)';

    }, 3000);

    // 监听过渡完成的事件transitionend,谁做过渡给谁加
    swiper_slide.addEventListener('transitionend', function () {
        if (num >= 3) {
            num = 0;
            swiper_slide.style.transition = "none";
